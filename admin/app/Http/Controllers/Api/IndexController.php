...
class IndexController extends Controller
{
    ...
    public function getAllGameList(Request $request)
    {
        ...
        $list = GameList::when($platform,function ($query) use ($platform){
            ...
            return $query->where('category_id',$category);
        })->where('is_top',1)->where('site_state',1)->select('name','platform_name','category_id','game_code','app_state','check_yes_img','check_no_img','api_logo_img','mobile_img','header_logo')->orderBy('order_by','asc')->get()->toArray();
		foreach($list as $key => $value){
